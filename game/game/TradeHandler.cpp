...
BOOL CTradeHandler::OnMouseMove( CMouse * pcMouse )
{
	if ( IsWindowTradeOpen() && !IsDelayTime() )
		return pWindow->OnMouseMove( pcMouse, 0, 0, DX::Graphic::GetWidthBackBuffer(), DX::Graphic::GetHeightBackBuffer(), 0, 0 );
...
void CTradeHandler::Render()
{
	if ( IsWindowTradeOpen() )
	{
		...
		if ( COINHANDLER->GetWindow()->IsOpen() || WINDOW_ISOPEN_PERSONALSHOP_OTHERS || WINDOW_ISOPEN_NPC_SHOP || WINDOW_ISOPEN_WAREHOUSE )
		{
			CloseTradeWindow();
			return;
		}
		...
		FONTHANDLER->Draw( 0, (WINDOW_TRADEMOVE - 384) + 228, 50 + 40, ((char*)0x03706F48), DT_LEFT, -1 );
