...
int CSkillDataHandler::OnAtalantaSkill( Skill * psSkill, int iLevel )
{
	...
	switch ( psSkill->iID )
	{
		case SKILLID_SummonTiger:
			...
			SKILLMANAGERHANDLER->AddSkillInfo( 2, "%s\r", SKILLMANAGERHANDLER->IsAgressive( SKILLID_SummonTiger ) ? "Agressive" : "Passive" );
			iLineCount++;
...
int CSkillDataHandler::OnKnightSkill( Skill * psSkill, int iLevel )
{
	...
	switch ( psSkill->iID )
	{
		case SKILLID_HolyConviction:
			SKILLMANAGERHANDLER->AddSkillInfo( 1, "Pet Settings:\r" );
			SKILLMANAGERHANDLER->AddSkillInfo( 2, "%s\r", SKILLMANAGERHANDLER->IsAgressive( SKILLID_SummonTiger ) ? "Automatic" : "Passive" );
			iLineCount++;
...
int CSkillDataHandler::OnPriestessSkill( Skill * psSkill, int iLevel )
{
	...
	switch ( psSkill->iID )
	{
		case SKILLID_IceElemental:
			SKILLMANAGERHANDLER->AddSkillInfo( 1, "Pet Settings:\r" );
			SKILLMANAGERHANDLER->AddSkillInfo( 2, "%s\r", SKILLMANAGERHANDLER->IsAgressive( SKILLID_IceElemental ) ? "Agressive" : "Passive" );
			iLineCount++;
