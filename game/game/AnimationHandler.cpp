...
{
	if( !CAnimationHandler::vAnimationData.empty() )
	{
...
BOOL CAnimationHandler::DecryptBoneFile( EXEModel * pcModelAnimation )
{
	if ( pcModelAnimation )
	{
		for ( UINT u = 0; u < pcModelAnimation->uMeshCount; u++ )
		{
			EXEMesh * pcMesh = pcModelAnimation->paMesh[u];
...
BOOL CAnimationHandler::InjectDynamicAnimation( struct EXEModelData * psModelData, EXEModel * pcModelAnimation, int iID, BOOL bLoop, int iClassFlag, int iAreaType, int iaEvent[4], int iItemType, int iTypeAnim )
{
	...
	EXEModel * pcModel = psModelData->psMotion->pcModel;
	...
	{
		EXEMesh * pcMeshA	= pcModelAnimation->paMesh[i];

		if ( pcMeshA )
		{
			//Get Parent Mesh
			EXEMesh * pcMesh = pcModel->GetMesh( pcMeshA->szName );
...
{	
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		EXEModel * pcModel			= LoadEXEModelPhysique( szObjectName );
...
RawSMDKeyRotation * InjectKeyRotation( RawSMDKeyRotation * pArray, int iArraySize, RawSMDKeyRotation & sInjectThis, int iFirstFrame, int iLastFrame, int & iIndexOut )
{
	...
	//DEBUG ("wtf failed to inject rotation at [%d,%d]", iFirstFrame, iLastFrame );
...
BOOL MergeKeyRotations( RawSMDObject * psRawObject )
{
	...
	{
		//DEBUG( "Merged Key Rotations (%d -> %d)", psRawObject->iKeyRotationCount, iNewSize );
...
BOOL MergeKeyPositions( RawSMDObject * psRawObject )
{
	...
	{
		//DEBUG( "Merged Key Positions (%d -> %d)", psRawObject->iKeyPositionCount, iNewSize );
...
BOOL MergeKeyScalings( RawSMDObject * psRawObject )
{
	...
	{
		//DEBUG( "Merged Key Scales (%d -> %d)", psRawObject->iKeyScaleCount, iNewSize );
...
void CAnimationHandler::PrintInfoINX( const char * pszFilePath )
{
	...
	//Load INX File
	//DEBUG( "Loading INX File %s", pszFilePath );
	if( !inxFile.Load() )
	{
		//DEBUG( "Cant Load INX File %s", pszFilePath );
		return;
	}
	...
	if( !ofs.is_open() )
	{
		//DEBUG( "Error! Print file is not open!" );
		inxFile.Unload();
		return;
	}
...
{
	//DEBUG( "FixSMBINXSubFileMotion(%s,%s) Working...", pszSMBFile, pszINXFile );
	...
	if( !impSMDFile.Load() )
	{
		//DEBUG( "FixSMBINXSubFileMotion() failed to load SMB" );
		return;
	}

	if( !impINXFile.Load() )
	{
		//DEBUG( "FixSMBINXSubFileMotion() failed to load INX" );
		return;
	}

	if( impSMDFile.sRawHeader.iObjectCount <= 0 )
	{
		//DEBUG( "FixSMBINXSubFileMotion() no objects in SMB, there's no point to this!" );
		return;
	}

	//DEBUG( "Frame Count: %d", impSMDFile.sRawHeader.iFrameCount );

	if( impSMDFile.sRawHeader.iFrameCount <= 1 )
	{
		//DEBUG( "FixSMBINXSubFileMotion() SMB Frame Count is already 0 or 1... there's no point to this!" );
		return;
	}
	...
	//Fix SMB Header
	{
		...
		for( int i = 1; i < impSMDFile.sRawHeader.iFrameCount; i++ )
		{
			...
			//DEBUG( "Frame[%d] : (%d,%d)", i, p->iStartFrame, p->iEndFrame );
	...
	//Fix INX Animations
	{
		...
		while( (i < 512) && (impINXFile.sRawFile.saAnimation[i].iType != 0) )
		{
			...
			if( iIndex < 0 )
			{
				//DEBUG( "Weird Index in INX Animation [%d] -> %d", i, iIndex );
				return;
			}

			if( iIndex >= psFirstObject->iFrameCount )
			{
				//DEBUG( "Too High Index in INX Animation [%d] : [%d] >= [%d]", i, iIndex, psFirstObject->iFrameCount );
				return;
			}
	...
	for( int i = 0; i < impSMDFile.sRawHeader.iObjectCount; i++ )
	{
		...
		if( psRawObject->szParentObjectName[0] != 0 )
		{
			...
			{
				//DEBUG( "Warning! Parent Object %s not found of Object %s", psRawObject->szParentObjectName, psRawObject->szObjectName );
			}
		}

		//DEBUG( "Object %s (Parent: %s)", psRawObject->szObjectName, psRawObjectParent ? psRawObjectParent->szObjectName : "None" );

		//DEBUG( "Object %s has %d Key Rotations", psRawObject->szObjectName, psRawObject->iKeyRotationCount );
		//DEBUG( "Object %s has %d Key Positions", psRawObject->szObjectName, psRawObject->iKeyPositionCount );
		//DEBUG( "Object %s has %d Key Scales", psRawObject->szObjectName, psRawObject->iKeyScaleCount );
		...
		//DEBUG( "Object %s default Key Rotation: W %.4f X %.4f Y %.4f Z %.4f", psRawObject->szObjectName, sKeyRotationTemp.fW, sKeyRotationTemp.fX, sKeyRotationTemp.fY, sKeyRotationTemp.fZ );
		...
		//DEBUG( "Object %s default Key Position: X %.4f Y %.4f Z %.4f", psRawObject->szObjectName, sKeyPosition.fX, sKeyPosition.fY, sKeyPosition.fZ );
		...
		//DEBUG( "Object %s default Key Scale: X %d Y %d Z %d", psRawObject->szObjectName, sKeyScale.iX, sKeyScale.iY, sKeyScale.iZ );
		...
		{
			//DEBUG( "Fixed Key Rotation Array of Object %s", psRawObject->szObjectName );
		...
		{
			//DEBUG( "Fixed Key Position Array of Object %s", psRawObject->szObjectName );
		...
		{
			//DEBUG( "Fixed Key Scaling Array of Object %s", psRawObject->szObjectName );
		...
		//DEBUG( "Object %s now has %d Key Rotations", psRawObject->szObjectName, psRawObject->iKeyRotationCount );
		//DEBUG( "Object %s now has %d Key Positions", psRawObject->szObjectName, psRawObject->iKeyPositionCount );
		//DEBUG( "Object %s now has %d Key Scales", psRawObject->szObjectName, psRawObject->iKeyScaleCount );

		if( bObjectKeysChanged )
		{
			//DEBUG( "Object %s Has Changed! Health Check...", psRawObject->szObjectName );
			...
			if( psRawObject->iKeyRotationCount > 0 )
			{
				...
				for( int a = 1; a < psRawObject->iKeyRotationCount; a++ )
				{
					...
					{
						//DEBUG( "Key Rotation [%d/%d] Invalid Frame [%d <= %d]", a, psRawObject->iKeyRotationCount, p->iFrame, prev->iFrame );
					}
			...
			if( psRawObject->iKeyPositionCount > 0 )
			{
				...
				for( int a = 1; a < psRawObject->iKeyPositionCount; a++ )
				{
					...
					if( p->iFrame < prev->iFrame )
					{
						//DEBUG( "Key Position [%d/%d] Invalid Frame [%d <= %d]", a, psRawObject->iKeyPositionCount, p->iFrame, prev->iFrame );
					}
			...
			if( psRawObject->iKeyScaleCount > 0 )
			{
				...
				for( int a = 1; a < psRawObject->iKeyScaleCount; a++ )
				{
					...
					if( p->iFrame < prev->iFrame )
					{
						//DEBUG( "Key Scale [%d/%d] Invalid Frame [%d <= %d]", a, psRawObject->iKeyScaleCount, p->iFrame, prev->iFrame );
					}
			...
			//DEBUG( "Object %s Health Check Finished", psRawObject->szObjectName );
		}
	}
	...
	//Done!
	//DEBUG( "FixSMBINXSubFileMotion(%s,%s) Finished!", pszSMBFile, pszINXFile );
}
...
{
	//DEBUG( "InjectionAnimation(%s,%s,%s) Working...", pszSMBFileFrom, pszSMBFileTo, pszINXFileTo );
	...
	if( !smbI1.Load() )
	{
		//DEBUG( "Can't load SMB: %s", pszSMBFileFrom );
		return FALSE;
	}

	if( !smbI2.Load() )
	{
		//DEBUG( "Can't load SMB: %s", pszSMBFileTo );
		return FALSE;
	}
	...
	if( !inxI.Load() )
	{
		//DEBUG( "Can't load INX: %s", pszINXFileTo );
		return FALSE;
	}
	...
	if( !bFoundKeyFrames )
	{
		//DEBUG( "No Animation Data (Key Frames) found in the source SMB file.. this is pointless!" );
		return FALSE;
	}
	...
	for( int i = 0; i < smbI1.sRawHeader.iObjectCount; i++ )
	{
		...
		{
			//DEBUG( "Object %s not found in Motion SMB, ignoring...", psRawObjectFrom->szObjectName );
			continue;
		}
		
		//DEBUG( "[%d][%d] Object %s Copying Animation Key Frames...", i, iNewObjectIndex, psRawObjectFrom->szObjectName );
		...
		//DEBUG( "Object %s Frame Rot Cnt: %d", psRawObjectFrom->szObjectName, psRawObjectFrom->iKeyRotationCount );
		//DEBUG( "Object %s Frame Tra Cnt: %d", psRawObjectFrom->szObjectName, psRawObjectFrom->iKeyPositionCount );
		//DEBUG( "Object %s Frame Scl Cnt: %d", psRawObjectFrom->szObjectName, psRawObjectFrom->iKeyScaleCount );
		...
		if( psRawObjectFrom->iKeyRotationCount > 0 )
		{
			...
			//DEBUG( "Copied %d Key Rotations for Frames %d,%d", psRawObjectFrom->iKeyRotationCount, iNewAnimationStartFrame, iNewLastFrame );
		}
		else
		{
			InjectKeyRotation( psRawObjectTo, -1, iNewAnimationStartFrame, &sKeyRotation, &sRotation );
			...
			//DEBUG( "Created Key Rotation for Frames %d,%d", iNewAnimationStartFrame, iNewLastFrame );
		}
		...
		if( psRawObjectFrom->iKeyPositionCount > 0 )
		{
			...
			//DEBUG( "Copied %d Key Positions for Frames %d,%d", psRawObjectFrom->iKeyPositionCount, iNewAnimationStartFrame, iNewLastFrame );
		}
		else
		{
			InjectKeyPosition( psRawObjectTo, -1, iNewAnimationStartFrame, &sKeyPosition );
			...
			//DEBUG( "Created Key Position for Frames %d,%d", iNewAnimationStartFrame, iNewLastFrame );
		}
		...
		if( psRawObjectFrom->iKeyScaleCount > 0 )
		{
			...
			//DEBUG( "Copied %d Key Scales for Frames %d,%d", psRawObjectFrom->iKeyScaleCount, iNewAnimationStartFrame, iNewLastFrame );
		}
		else
		{
			InjectKeyScale( psRawObjectTo, -1, iNewAnimationStartFrame, &sKeyScale );
			...
			//DEBUG( "Created Key Scale for Frames %d,%d", iNewAnimationStartFrame, iNewLastFrame );
		}
		...
		//DEBUG( "Object %s Done!", psRawObjectFrom->szObjectName );
	}
	...
	//Log
	//DEBUG( "Injected INX Animation at Index %d", inxI.sRawFile.uAnimationCount );
