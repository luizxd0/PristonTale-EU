...
void CCoinHandler::OpenCoinShopNPC()
{
...
void CCoinHandler::HandlePacket( struct PacketCoinAmount * psPacket )
{
	...
	COINHANDLER->GetWindow()->SetCoinAmount( ACCOUNTHANDLER->GetCoinAmount() );
}
...
void CCoinHandler::HandlePacket( struct PacketCoinBuy * psPacket )
{
	switch ( psPacket->iReturnCode )
	{
		...
		case 1:
			ChatGame::AddChatBoxText( "> Item bought! Please visit the item distributor to receive your item.", CHATCOLOR_Global );
			TITLEBOX( "Item bought! Please visit the item distributor to receive your item." );
			PLAYMINISOUND( 25 );
...
{
	if ( psPacket->iType == ITEMCOIN_ItemPerfect || psPacket->iType == ITEMCOIN_ItemSwapper ||
