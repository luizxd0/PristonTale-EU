...
static __declspec(naked) EXEModel * LoadEXEModelPhysique( const char * pszFile )
{
	JMP( 0x0047B360 );
}
...
public:
	...
	virtual BOOL							DecryptBoneFile( EXEModel * pcModelAnimation );

	BOOL									InjectDynamicAnimation( struct EXEModelData * psModelData, EXEModel * pcModelAnimation, int iID, BOOL bLoop, int iClassFlag, int iAreaType, int iaEvent[4], int iItemType, int iTypeAnim );
