...
{
	return CQUESTGAME->GetQuestInstance()->IsDoneQuest( dwQuestID );
}

BOOL QuestGame::IsActiveQuest( DWORD dwQuestID )
{
	return CQUESTGAME->GetQuestInstance()->IsActiveQuest( dwQuestID );
}
...
const std::string QuestGame::GetQuestMonsterNameFromID(DWORD iUniqueMonsterID)
{
	if ( iUniqueMonsterID == QUEST_MONSTERID_ANY )
		return "Monsters";

	auto it = mMonsterName.find( iUniqueMonsterID );

	if ( it != mMonsterName.end() )
		return it->second;

	if ( dwNextMonsterNameReqTime == 0 || TICKCOUNT > dwNextMonsterNameReqTime )
	{
		SENDCOMMANDG( PKTHDR_QuestMonsterNameReq, iUniqueMonsterID );
		dwNextMonsterNameReqTime = TICKCOUNT + 5000;
	}

	return "";
}

void QuestGame::HandlePacket( PacketQuestMonsterName* psPacket )
{
	if ( mMonsterName.find( psPacket->uMonsterUniqueID ) != mMonsterName.end() )
		return;

	ITEMINFOBOX->Invalidate();

	mMonsterName.insert( { psPacket->uMonsterUniqueID, std::string( psPacket->szMonsterName ) } );
}

void QuestGame::HandlePacket( PacketTransCommand * psPacket )
{
	if ( psPacket->iHeader == PKTHDR_NpcQuestStatus )
	{
		//CHATBOX->AddMessage( FormatString( "NPC ID: %d: %d", psPacket->LParam, psPacket->WParam ) );

		auto it = mNpcToQuestStatus.find( psPacket->LParam );

		if ( it == mNpcToQuestStatus.end() )
			mNpcToQuestStatus.insert( { psPacket->LParam, (EQuestIcon)psPacket->WParam } );
		else
			it->second = (EQuestIcon)psPacket->WParam;
	}
}

EQuestIcon QuestGame::GetNpcQuestStatus( int iNPCId )
{
	auto it = mNpcToQuestStatus.find( iNPCId );

	if ( it == mNpcToQuestStatus.end() )
		return EQuestIcon::None;
