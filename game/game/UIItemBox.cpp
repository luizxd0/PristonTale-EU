...
BOOL ItemBox::OnMouseMove( class CMouse * pcMouse, int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY )
{
	return FALSE;
}
			...
			if ( ps )
			{
				...
				if ( ps->eRule == eRule )
				{
					...
					if ( ps->eType == TYPERULE_ItemPerfect )
						if ( pcItemData->sItem.iItemSpecialType == ps->iParam )
							bRet = TRUE;
...
BOOL ItemBox::AddItem(ItemData * pcItemData, int iX, int iY, int iMouseX, int iMouseY, BOOL bPlaySound )
{
