...
BOOL DXGeometryInstance::Init()
{
	...
	if ( (hr = lp->CreateVertexBuffer( sizeof( VertexGeometry ) * 16384, 0, 1220, D3DPOOL_MANAGED, &lpVBRendererModelData, NULL )) != D3D_OK )
	{
		return FALSE;
	}

	if ( (hr = lp->CreateVertexBuffer( sizeof( VertexInstance ) * 16384, 0, 1220, D3DPOOL_MANAGED, &lpVBRendererInstanceData, NULL )) != D3D_OK )
	{
		return FALSE;
	}

	if ( (hr = lp->CreateIndexBuffer( 3 * 18000 * sizeof( WORD ), 0, D3DFMT_INDEX16, D3DPOOL_MANAGED, &lpIBRenderer, NULL )) != D3D_OK )
	{
		return FALSE;
	}
