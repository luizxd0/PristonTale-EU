...
BOOL ComboBox::OnMouseClick( class CMouse * pcMouse, int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY )
{
	...
	if( pcMouse->GetEvent() == EMouseEvent::ClickDownL && rBox.Inside( pcMouse->GetPosition() ) )
	{
		bShowMenu = TRUE;
		return TRUE;
	}
	else if( pcMouse->GetEvent() == EMouseEvent::ClickDownL && bShowMenu && rMenu.Inside( pcMouse->GetPosition() ) )
	{
		//Invoke Change Event
		if( iEventID != -1 )
	...
	else if( pcMouse->GetEvent() == EMouseEvent::ClickDownL )
		bShowMenu = FALSE;
