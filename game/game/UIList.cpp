...
Scroll_ptr List::MakeScrollBar( Rectangle2D sBox, std::string strScrollBarPath, std::string strScrollerPath, int iScrollerWidth, int iScrollerHeight )
{
	...
	pScrollBar->SetScroller( UI::ImageLoader::LoadImage( strScrollerPath.c_str() ), iScrollerWidth, iScrollerHeight );
...
BOOL List::OnMouseClick( class CMouse * pcMouse, int iX, int iY, int iWidth, int iHeight, int iSourceX, int iSourceY )
{
	...
	if( pScrollBar )
	{
		if (pScrollBar->OnMouseClick( pcMouse, GetX() + iX, GetY() + iY, iWidth, iHeight, iSourceX, iSourceY ))
			bRet = TRUE;
	}
	...
	if ( rBox.Inside( pcMouse->GetPosition() ) )
	{
		...
		for( auto &v : vWindows )
		{
			if( bNoUpdatePosition )
			{
				if ( v->OnMouseClick( pcMouse, iRenderX, iRenderY - iScroll, iRenderWidth, iRenderHeight + iScroll, 0, 0 ) )
					bRet = TRUE;
			...
			if( rBoxWindow.Inside( pcMouse->GetPosition() ) )
			{
				...
				if ( v->OnMouseClick( pcMouse, rBoxWindow.iX, rBoxWindow.iY, rBoxWindow.iWidth, rBoxWindow.iHeight, iSourceX, iSourceY ) )
				{
					bRet = TRUE;
					break;
				}
	...
	{
		for ( auto & v : vWindows )
			v->OnMouseClick( pcMouse, 0, 0, 0, 0, iSourceX, iSourceY );
	}
