...
int CChristmasHandler::IsPremiumChristmas( int iItemID )
{
	BOOL bResult = FALSE;
	switch ( iItemID )
	{
...
void CChristmasHandler::OnClickBox( UIEventArgs e )
{
	if ( MESSAGEBOX->GetType() )
	{
		...
		PacketNPCRudolphChristmas s;
		s.iLength = sizeof( PacketNPCRudolphChristmas );
		s.iHeader = PKTHDR_NPCRudolphChristmas;
...
{
	//TODO - get Christmas status from server
...
{
	//TODO - get Christmas status from server
	if ( Game::GetGameMode() == GAMEMODE_InGame && CHARACTERGAME->IsStageVillage() )
	{		
