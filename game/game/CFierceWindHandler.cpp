...
bool CFierceWindHandler::OnCastAnimationStart( struct Skill * psSkill, class UnitData * pcUnitData )
{
	...
	if ( p )
	{
		if ( USESKILL( psSkill ) )
		{
			...
			SKILLMANAGERHANDLER->BeginSkill( UNITDATA, UNITDATA->sActiveSkill.iValue, psSkill->iLevel, UNITDATA->pcTarget, UNITDATA->sPosition.iX, UNITDATA->sPosition.iY, UNITDATA->sPosition.iZ );
			...
			{ 
				if ( pcUnitData->GetAnimationAttackPosition( &s.iX, &s.iY, &s.iZ ) )
				{
					...
					if ( pcUnitData == UNITDATA )
					{
						...
						SKILLMANAGERHANDLER->SelectRangeBox( UNITDATA, rRect );

						SKILLMANAGERHANDLER->SendRangeDamage( pcTarget, pcTarget->sPosition, 0, 0, 0, 0, UNITDATA->sActiveSkill.iValue );
					}
					return TRUE;
				}
			}
