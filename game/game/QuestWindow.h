...
public:
	...
	BOOL							IsOtherWindowsOpen();
	BOOL							IsClassicNPCWindowOpen();

	void							Init();
	void							UpdateMainWindowPosition();
	void							UpdateItemWindowPosition();

	void							Open( int iID, char * pszName, char * pszDescription, int iNPCType, int iAutoRewardItemId = 0 );
	...
	int								GetItemCodeAtIndex( int iIndex );
	ItemData *						GetItemDataAtIndex( int iIndex );
