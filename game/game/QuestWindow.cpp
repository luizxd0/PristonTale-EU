...
void CQuestWindow::OnButtonOKClick( UIEventArgs e )
{
	if ( CQUESTGAME->IsFinishing() == FALSE )
	{
		...
		if ( pcQuestData == NULL )
		{
			...
			MESSAGEBOX->SetDescription( FormatString("Do you want to accept the %s quest?", szName) );
		...
		else
		{
			if ( pcQuestData->IsFinished() && (iNPCType == 3 || iNPCType == 4) )
			{
				...
				{
					if ( iAutoRewardItemId > 0 )
					{
						if ( !CHARACTERGAME->CheckInventorySpace( ITEMHANDLER->FindItemTableByCode(iAutoRewardItemId ) ) )
						{
...
{
	if ( !CHARACTERGAME->CheckInventoryWeight( GetItemDataAtIndex( iItemIndex ) ) )
	{
		TITLEBOX( "Weight limit exceeded" );
		return;
	}

	if ( !CHARACTERGAME->CheckInventorySpace( ITEMHANDLER->FindItemTableByCode( GetItemCodeAtIndex( iItemIndex ) ) ) )
	{
		TITLEBOX( "Not enough space in inventory" );
		return;
	}
...
const std::string CQuestWindow::GetQuestWaitMessage(EQuestType eQuestType)
{
	switch ( eQuestType )
	{
...
void CQuestWindow::OnButtonOpenDynamicQuestWindow( int iID, UIEventArgs e )
{
	...
	{
		for ( auto q : vDynamicQuests )
		{
			if ( q->iQuestID == iID)
			{
				if ( q->eQuestUserStatus == EQuestUserStatus::Expired || q->eQuestUserStatus == EQuestUserStatus::NotStarted )
				{
					//all good to start.
					break;
				}

				//unexpected states. send packet to server and it might return with an error message anyway.
				if ( q->eQuestUserStatus == EQuestUserStatus::Completed ||
					 q->eQuestUserStatus == EQuestUserStatus::Error ||
					 q->eQuestUserStatus == EQuestUserStatus::InProgress )
				{
					break;
				}

				//quest is done
				if ( q->eQuestUserStatus == EQuestUserStatus::Done )
				{
					if ( q->eQuestType == EQuestType::SoloOneOff || q->eQuestType == EQuestType::PartyOneOff )
					{
						TITLEBOX("You have already completed this one-off quest!");
						return;
					}

					if ( q->eQuestType == EQuestType::Repeatable || q->eQuestType == EQuestType::WeekendRepeatable )
					{
						//all good to start.
						break;
					}

					if ( q->eQuestType == EQuestType::None )
					{
						//huh?
						break;
					}

					TITLEBOX(GetQuestWaitMessage(q->eQuestType).c_str());
					return;
				}
			}
		}
...
BOOL CQuestWindow::IsOtherWindowsOpen()
{
	if ( GRANDFURYHANDLER->IsNpcWindowOpen() )
		return TRUE;

	return FALSE;
}

BOOL CQuestWindow::IsClassicNPCWindowOpen()
{
	//ASM: cSinHelp.sinGetHelpShowFlag()
	if( (bool)CALLT(0x0049CD90, 0x35E05D8) == true )
		return TRUE;

	if ( WINDOW_ISOPEN_WAREHOUSE ||
		 WINDOW_ISOPEN_SMELTING ||
		 WINDOW_ISOPEN_NPC_SHOP ||
		 WINDOW_ISOPEN_MIXING ||
		 WINDOW_ISOPEN_CRAFTING ||
		 WINDOW_ISOPEN_AGING ||
		 WINDOW_ISOPEN_RESETMIX ||
		 WINDOW_ISOPEN_SKILLMASTER ||
		 WINDOW_ISOPEN_CLANMANAGER ||
		 WINDOW_ISOPEN_BELLATRA_RANKING)
	{
		return TRUE;
	}
...
void CQuestWindow::UpdateMainWindowPosition()
{
	if ( pWindow == NULL )
		return;
	...
	//int iWindowX = ( RESOLUTION_WIDTH >> 2 ) - ( iWindowWidth >> 1 ) + 30;
	//int iWindowY = ( RESOLUTION_HEIGHT >> 2 ) - ( iWindowHeight >> 1 ) + 78;
	int iWindowX = ( RESOLUTION_WIDTH / 3 ) - ( iWindowWidth >> 1 );
	int iWindowY = ( RESOLUTION_HEIGHT >> 1 ) - ( iWindowHeight >> 1 ) - 60;

	if ( pDynamicWindow && pDynamicWindow->IsOpen() )
	{
		iWindowX = pDynamicWindow->GetX() + pDynamicWindow->GetWidth() + 20;
		iWindowY = pDynamicWindow->GetY();
	}
	else if ( IsClassicNPCWindowOpen() || IsOtherWindowsOpen() )
	{
		iWindowX = 400;
	}
...
void CQuestWindow::UpdateItemWindowPosition()
{
	if ( pWindowItems == NULL )
		return;
...
void CQuestWindow::Open( int iID, char * pszName, char * pszMessage, int iType, int iAutoRewardItemId )
{
	iCurrentID = iID;
	this->iNPCType = iType;
	this->iAutoRewardItemId = iAutoRewardItemId;

	if ( IsNull( pszMessage ) && iType == 3 ) //Is Conlusion, but has no conclusion text.
	{
		OnButtonOKClick( UIEventArgs{} );
	}
	else
	{
...
{
	if ( PLAYERDATA->iLevel > psQuestData->sRequiredLevel.iMax )
		return;
	...
	ps->eQuestUserStatus = psQuestData->eQuestUserStatus;
	ps->eQuestType       = psQuestData->eQuestType;
...
void CQuestWindow::OpenDynamicQuestWindow()
{
	...
	BOOL bSortingNeeded = FALSE;
	int iLastReqLevel = -1;
	for ( auto ps : vDynamicQuests )
	{
		if ( ps->sRequiredLevel.iMin < iLastReqLevel )
		{
			bSortingNeeded = TRUE;
			break;
		}

		iLastReqLevel = ps->sRequiredLevel.iMin;
	}

	if ( bSortingNeeded )
	{
		std::sort( vDynamicQuests.begin(), vDynamicQuests.end(), []( const PacketDynamicQuestOpen::QuestData * lhs, const PacketDynamicQuestOpen::QuestData * rhs )
		{
			if ( lhs->sRequiredLevel.iMin < rhs->sRequiredLevel.iMin ) return true;
			if ( lhs->sRequiredLevel.iMin > rhs->sRequiredLevel.iMin ) return false;

			if ( lhs->sRequiredLevel.iMax < rhs->sRequiredLevel.iMax ) return true;
			if ( lhs->sRequiredLevel.iMax > rhs->sRequiredLevel.iMax ) return false;
	...
	//show the top 10 quests only
	int iTotalAvaliableQuests = 0;
	for ( auto ps : vDynamicQuests )
	{
		if ( ps && PLAYERDATA->iLevel >= ps->sRequiredLevel.iMin && PLAYERDATA->iLevel <= ps->sRequiredLevel.iMax )
			iTotalAvaliableQuests++;
	}
	...
	for ( auto ps : vDynamicQuests )
	{
		...
		BOOL bIsWithinReqLevelRange = PLAYERDATA->iLevel >= ps->sRequiredLevel.iMin && PLAYERDATA->iLevel <= ps->sRequiredLevel.iMax;
		if ( bIsWithinReqLevelRange )
		{
			iCounter++;

			if ( iCounter <= (iTotalAvaliableQuests - 10) ) //only show top 10 quests
				continue;
		}
		...
		UI::Window_ptr pMenuWindow = std::make_shared<UI::Window>( Rectangle2D( 0, 0, pDynamicWindow->GetWidth(), 26 ) );
		{
			...
			if ( bIsWithinReqLevelRange && (ps->eQuestUserStatus == EQuestUserStatus::NotStarted || ps->eQuestUserStatus == EQuestUserStatus::Expired) )
				pTextMenu->SetFont( "Arial", 16, 0, FALSE, FALSE, D3DCOLOR_XRGB( 255, 255, 255 ) );
			else
				pTextMenu->SetFont( "Arial", 16, 0, FALSE, FALSE, D3DCOLOR_XRGB( 150, 150, 150 ) );

			if ( (ps->sRequiredLevel.iMin > 0) || (ps->sRequiredLevel.iMax > 0) )
			{
				if ( ps->sRequiredLevel.iMax == 150 )
					pTextMenu->SetTextEx( "%s (Lv. %d+)", ps->szQuestName, ps->sRequiredLevel.iMin);
				else
					pTextMenu->SetTextEx( "%s (Lv. %d-%d)", ps->szQuestName, ps->sRequiredLevel.iMin, ps->sRequiredLevel.iMax);
			}
			else
...
int CQuestWindow::GetItemCodeAtIndex( int iIndex )
{
	UI::List_ptr pListItems = pWindowItems->GetElement<UI::List>( LISTID_Items );

	UI::Window_ptr pWindow = pListItems->GetWindow( iIndex );

	if ( pWindow )
	{
		UI::ItemBox_ptr pItemBox = pWindow->GetElement<UI::ItemBox>( 1 );

		if ( pItemBox->GetItems().size() == 0 )
			return 0;

		return pItemBox->GetItems()[0]->sItem.sItemID.ToInt();
	}

	return 0;
}

ItemData * CQuestWindow::GetItemDataAtIndex( int iIndex )
{
	UI::List_ptr pListItems = pWindowItems->GetElement<UI::List>( LISTID_Items );
