...
void CManageWindow::UpdateClanIcons()
{
	...
	for ( int i = 0; i < 500; i++ )
	{
		...
		if ( ps->iStatus == 2 )
		{
			...
			{
				STRINGFORMAT( szClanID, "10_%06d", ps->sHeader.dwClanTextureNum - 1000000000 );
				STRINGFORMAT( szPath, "save\\clanDATA\\%s\\%s_16.bmp", szServerName, szClanID );
			...
			{
				STRINGFORMAT( szClanID, "10_%06d", ps->sHeader.dwClanTextureNum - 1000000000 );
				STRINGFORMAT( szPath, "save\\clanDATA\\%s\\%s.bmp", szServerName, szClanID );
