	...
	if ( pcUnitData->GetAnimationAttackPosition( &s.iX, &s.iY, &s.iZ ) )
	{
		...
		else
		{
			...
			if ( pcUnitData == UNITDATA )
			{
				...
				{
					//Last Hit is AoE
					if ( pcUnitData->iMotionEvent == 4 )
					{
						SKILLMANAGERHANDLER->SetUnitsRange( pcUnitData->pcTarget->sPosition, 90 );

						SKILLMANAGERHANDLER->SendRangeDamage( pcUnitData->pcTarget, pcUnitData->pcTarget->sPosition, 0, 0, 0, 0, UNITDATA->sActiveSkill.iValue );
					}
					else
					{
						SKILLMANAGERHANDLER->SendTransDamage( pcUnitData->pcTarget, UNITDATA->sActiveSkill );
					}
				}
			}
