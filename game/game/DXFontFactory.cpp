...
{
	auto vFiles = CSystem::GetInstance()->ListFilesInDirectory( "game\\fonts\\", "ttf" );
...
Font_ptr FontFactory::MakeFont( Sprite_ptr pSprite, std::string strFontName, int Height, int Width, BOOL bBold, BOOL bItalic )
{
	...
	d3dxFontDesc.Quality			= DEFAULT_QUALITY;
	...
	//New Font
	auto pFont = std::make_shared<Font>( pSprite, lpD3DXFont, d3dxFontDesc );
