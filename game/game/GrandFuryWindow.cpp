...
void CGrandFuryWindow::BuildWindow1()
{
	...
	pTextTeleportCT2->SetText( "Teleport to Fury Arena Entrance" );
	...
	pTextTeleportFA->SetText( "Teleport to Fury Arena" );
...
void CGrandFuryWindow::OnTeleportCT2ButtonClick( UIEventArgs e )
{
	...
	int iCharLevel = UNITDATA->sCharacterData.iLevel;
	int iPrice = iCharLevel * 1000;
	//int iPrice = round((iCharLevel*pow(10,(0.02*iCharLevel-(0.02*75)))*1000)/5000)*5000;
	...
	if ( iPrice <= UNITDATA->sCharacterData.iGold )
	{
		...
		if ( iLevel <= UNITDATA->sCharacterData.iLevel )
		{
			...
			MESSAGEBOX->SetDescription( FormatString( "Do you want to teleport for Fury Arena?\nPrice: %s", szTempText ) );
...
void CGrandFuryWindow::OnTeleportFuryArenaButtonClick( UIEventArgs e )
{
	...
	if ( dw >= (5) && dw <= (15 * 60) )
	{
		...
		TITLEBOX( "You cannot enter the Arena at this moment." );
}
...
void CGrandFuryWindow::OnAcceptExchangeButtonClick( UIEventArgs e )
{
	if ( MESSAGEBOX->GetType() == TRUE )
	{
		...
		if ( ps )
		{
			...
			sPacket.p1		= 100;
			sPacket.p4		= (int)ps->sItem.eCraftType;
			sPacket.p5		= ps->sItem.tTime;
			SENDPACKETL( &sPacket, TRUE );
...
void CGrandFuryWindow::Open( int iNPC )
{
	if ( !pWindow1->IsOpen() )
	{
		...
		{
			pTextBossName->SetText( FormatString( "%s", GRANDFURYHANDLER->GetEventID() == 0 ? "King Fury" : "Ancient Fury" ) );
			pTextBossLevel->SetText( FormatString( "%s", GRANDFURYHANDLER->GetEventID() == 0 ? "Level: 95+" : "Level: 75 - 94" ) );
			...
			{
				pImageBoss->SetImage( UI::ImageLoader::LoadImage( "game\\images\\events\\grandfury\\babel.png" ) );
			}
