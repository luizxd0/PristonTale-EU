...
void CUIControllerGeneral::Render()
{
	...
	if( READDWORD(0x0367E83C) > 0 )
	{
		...
		{
			FONTHANDLER->Draw( 0, RECT{ 229, (RESOLUTION_HEIGHT + 56) - WINDOW_SKILLMOVE + 175, 0, 0 }, FormatString( "%d", CHARACTERGAME->GetFreeSkillPointForTier5() ), DT_NOCLIP, -1 );
		}
		else
		{
			//SP & EP
	...
	if( WINDOW_ISOPEN_SKILLMASTER > 0 )
	{
		...
		{
			WRITEDWORD( 0x0362BA7C, 1 );
			WRITEDWORD( 0x362BA90, 1 );
			WRITEDWORD( 0x367DE90, 0 );
		}

		FONTHANDLER->Draw( 0, RECT{ 112, 275, 0, 0 }, FormatString( "Future Skill Points: %d", CHARACTERGAME->GetFreeSkillPointForTier5() ), DT_NOCLIP, D3DCOLOR_XRGB( 230, 170, 40 ) );

		int iPoints = READDWORD( 0x03673B70 ) + READDWORD( 0x03673B74 ) + CHARACTERGAME->GetFreeSkillPointForTier5();
