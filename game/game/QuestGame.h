...
private:
	...
	DWORD										dwNextMonsterNameReqTime = 0;

	std::map<DWORD, EQuestIcon>					mNpcToQuestStatus;
	std::map<DWORD, const std::string>			mMonsterName;
...
public:
	...
	const std::string							GetQuestMonsterNameFromID( DWORD iUniqueMonsterID );

	void										HandlePacket( PacketQuestMonsterName * psPacket );
	void										HandlePacket( PacketTransCommand * psPacket);
	EQuestIcon									GetNpcQuestStatus( int iNPCId );
