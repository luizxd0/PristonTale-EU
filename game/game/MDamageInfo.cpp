...
void CDamageInfoModel::OnActionEvent( UINT uPosition, UIEventArgs eArgs )
{
	...
	else if ( vTargetsInfo[ uPosition ]->isDamage )
	{
		...
		int iWidth = FONTHANDLER->GetWidthText(iFontInfoDamage, szBuf, lstrlenA(szBuf)) >> 1;
...
void CDamageInfoModel::Load()
{
	...
	iFontInfoDamage = FONTHANDLER->CreateFont( "Arial", 42, 9, TRUE, FALSE, -1 );
	iFontInfoCritical = FONTHANDLER->CreateFont( "Arial", 52, 9, TRUE, FALSE, -1 );
