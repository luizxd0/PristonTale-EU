...
BOOL CImpulsionHandler::OnEvent( UnitData * pcUnitData )
{
	...
	if ( pcUnitData->GetAnimationAttackPosition( &s.iX, &s.iY, &s.iZ ) )
	{
		...
		SKILLMANAGERHANDLER->SelectRangeChained ( pcUnitData,
			pcUnitData->pcTarget,
			SKILLMANAGERHANDLER->GetSkillIntValue ( ESkillArrayPointer::Mech_Impulsion_MaxVolts, ESkillID::SKILLID_Impulsion ),
			SKILLMANAGERHANDLER->GetSkillIntValue ( ESkillArrayPointer::Mech_Impulsion_Area, ESkillID::SKILLID_Impulsion ) );
