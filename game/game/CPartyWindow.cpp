...
void CPartyWindow::Init()
{
	...
	for ( int i = 0; i < 10; i++ )
	{
		char * pszClass = GetCharacterClassStringByFlag( CharacterClassToClassFlag( (ECharacterClass)(i + 1) ) );
	...
	if( pListPartyMembers )
	{
		...
		for( size_t i = 0; i < MAX_PARTY_MEMBERS; i++ )
		{
			...
			if( sPartyMember.sData.iID )
			{
				...
				char * pszClass = GetCharacterClassStringByFlag( CharacterClassToClassFlag( (ECharacterClass)sPartyMember.cCharacterClass ) );
				...
				pImageCharacterClass->SetActiveImage(
                    UI::ImageLoader::LoadActiveImage(   (char*)(( "game\\images\\UI\\hud\\party\\" + std::string(pszClass) + ".png" )).c_str(),
                                                        (char*)(( "game\\images\\UI\\hud\\party\\" + std::string(pszClass) + "_.png" )).c_str()
                                                    )
                );
				...
				if( pcParty->saMembers[0].sData.iID == UNITDATA->iID )
				{
					...
					else
					{
						...
						//if (pcParty->iRaidCount > 0)
						//{
							//pDropdownMenu->AddOption(pDropdownMenu->MakeOption("Disband Raid", Rectangle2D(13, 5, 77, 18), FALSE, -1, D3DCOLOR_XRGB(230, 180, 150)), FALSE, 7);
						//}
						//else
						//{
							pDropdownMenu->AddOption(pDropdownMenu->MakeOption("Disband Party", Rectangle2D(13, 5, 77, 18), FALSE, -1, D3DCOLOR_XRGB(230, 180, 150)), FALSE, 3);
						//}

						pDropdownMenu->AddOption( pDropdownMenu->MakeOption( "Leave Party", Rectangle2D( 13, 5, 77, 18 ), FALSE, -1, D3DCOLOR_XRGB( 230, 180, 150 ) ), FALSE, 4 );
				...
				else
				{
					...
					{
						//Options
						pDropdownMenu->AddOption( pDropdownMenu->MakeOption( "Leave Party", Rectangle2D( 13, 5, 77, 18 ), FALSE, -1, D3DCOLOR_XRGB( 230, 180, 150 ) ), FALSE, 6 );
					}
				}
...
void CPartyWindow::ShowMessageBox( int iType, int iIndex )
{
	...
	{
		MESSAGEBOX->SetDescription("Do you want to disband the Raid?");
