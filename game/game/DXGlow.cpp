...
void DXGlow::CreateRenderBuffers()
{
	if ( !lpRenderTexture )
	{
		...
		//Render Target Glow
		if ( FAILED( hr = DX::Graphic::GetDevice()->CreateTexture( GRAPHICENGINE->Get3DPP().BackBufferWidth, GRAPHICENGINE->Get3DPP().BackBufferHeight, 1, D3DUSAGE_RENDERTARGET, GRAPHICENGINE->Get3DPP().BackBufferFormat, D3DPOOL_DEFAULT, &lpRenderTextureGlow, NULL ) ) )
		{
			GRAPHICENGINE->Log( "Could not create Render Target" );
			return;
		}
