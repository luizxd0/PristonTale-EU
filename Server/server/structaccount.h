...
enum ELogAccount
{
	...
	ACCLOGID_Unused									= iLogAccountBase + 12,
	...
	ACCLOGID_CharacterExpLose						= iLogAccountBase + 305,


	ACCLOGID_CharacterLoad							= iLogAccountBase + 306,

};

//											Definitions
#define LOGIN_SUCCESS_LOGACCTEXT						"[%s] (Login Success) -> Password[%s] MacAddr[%s] PCName[%s] SerialHD[%d] VideoName[%s]"
#define INCORRECT_ACCOUNT_LOGACCTEXT					"Unknown_Account (Incorrect Account) -> AccountName[%s] Password[%s] MacAddr[%s] PCName[%s] SerialHD[%d] VideoName[%s]"
