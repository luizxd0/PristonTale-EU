...
BOOL CBotServerHandler::CreateBot(const char * pszCharName, int iX, int iY, int iZ )
{
...
BOOL CBotServerHandler::DeleteBot(const char * pszCharName )
{
	for ( int i = 0; i < _MAX_BOTSERVER; i++ )
	{
...
BOOL CBotServerHandler::IsOnlineBot( const char * pszCharName )
{
	for ( int i = 0; i < _MAX_BOTSERVER; i++ )
	{
		SBotServer * psBotData = sBotData + i;
...
void CBotServerHandler::Tick()
{
	for ( int i = 0; i < _MAX_BOTSERVER; i++ )
	{
		...
		if ( psBotData->bValid && psBotData->dwTime < TICKCOUNT )
		{
			...
			sPacketGroup.iHeader = PKTHDR_UnitStatusContainer;
