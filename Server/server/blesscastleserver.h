...
public:
	...
	void												SetBlessCastleOwnerClanID( int iClanID1, int iClanID2 = 0, int iClanID3 = 0 );

	void												NetSendBlessCastleOwnerClanID( int iClanID1, int iClanID2 = 0, int iClanID3 = 0 );
	...
	BOOL												SelectDamageClanTop10 ( BlessCastleStatusData * lpBlessCasterStatus );

	ESiegeWarMode										GetSiegeWarMode();
	void												SetSiegeWarMode( ESiegeWarMode iNewMode );

	BOOL												Start();
	static BOOL											End( BOOL bUnitsAlive );

	static BOOL											EasySiegeWar();

	void												ResetUserScore();

	int													GetClanTop( int iTopNumber );

	void												SendTopClansSync( User * pcUser );
