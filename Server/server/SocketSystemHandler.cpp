...
void CSocketSystemHandler::OnMixingItem( User * pcUser, PacketMixServer * psPacket )
{
	...
	if ( psItemDefinition )
	{
		...
		{
			Item sItem;
			ITEMSERVER->CreateItem( &sItem, psItemDefinition, EItemSource::MixedSocket, 0);
			...
			CHATSERVER->SendTitleBox( pcUser->pcUserData, "Received %s", psPacket->sItemData.sItem.szItemName );
		}
	}
...
void CSocketSystemHandler::OnSocketDrill( User * pcUser, PacketSocketSystem * psPacket )
{
	...
	for ( int i = 0; i < iStoneCount; i++ )
	{
		...
		if (!ITEMSERVER->DeleteItemInventory( pcUser->pcUserData, ps->sBaseItemID.ToItemID(), ps->sItem.iChk1, ps->sItem.iChk2 ))
		{
			//Error
		}
	}
	...
	//Check Item validate
	if (!ITEMSERVER->DeleteItemInventory( pcUser->pcUserData, psItemData->sBaseItemID.ToItemID(), psItemData->sItem.iChk1, psItemData->sItem.iChk2 ))
	{
		//Error
	}
	...
	{
		// 5% Prismatic, 19% all
		if (Dice::RandomI( 0, 99 ) < 5 )
			eType = ESocketType::SOCKETTYPE_Prismatic;
		else
			eType = (ESocketType)Dice::RandomI( ESocketType::SOCKETTYPE_Emerald, ESocketType::SOCKETTYPE_Count-2 );
	}
	else //Fail
	{
	...
	ITEMSERVER->ReformItem( &psItemData->sItem );

	if (!ITEMSERVER->AddItemInventory( pcUser->pcUserData, psItemData->sBaseItemID.ToItemID(), psItemData->sItem.iChk1, psItemData->sItem.iChk2 ))
	{
	...
	if ( bSuccess )
		CHATSERVER->SendTitleBox( pcUser->pcUserData, "Socket Creation Success" );
	else
		CHATSERVER->SendTitleBox( pcUser->pcUserData, "Socket Creation Failed" );
...
void CSocketSystemHandler::OnSocketStone( User * pcUser, PacketSocketSystem * psPacket )
{
	...
	if ( eGemItem != 0 && eGemType != 0 )
	{
		...
		if (!ITEMSERVER->DeleteItemInventory( pcUser->pcUserData, pcItemData->sBaseItemID.ToItemID(), pcItemData->sItem.iChk1, pcItemData->sItem.iChk2 ))
		{
		...
		if (!ITEMSERVER->AddItemInventory( pcUser->pcUserData, pcItemData->sBaseItemID.ToItemID(), pcItemData->sItem.iChk1, pcItemData->sItem.iChk2 ))
		{
	...
	if ( bSuccess )
		CHATSERVER->SendTitleBox( pcUser->pcUserData, "Socket Stone Success" );
	else
		CHATSERVER->SendTitleBox( pcUser->pcUserData, "Socket Stone Failed" );
}
