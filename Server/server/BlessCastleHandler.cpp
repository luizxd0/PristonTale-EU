	...
	if ( GAME_SERVER && SERVER_CODE == 1 )
	{
		...
		if ( BLESSCASTLESERVER->GetSiegeWarMode() == SIEGEWARMODE_TheConquest || BLESSCASTLESERVER->GetSiegeWarMode() == SIEGEWARMODE_TheMassacre )
		{
			...
			if ( (ps->wDayOfWeek == 0) && (sLastDay != ps->wDay) )
			{
				...
				//JLM _ tax rate on server side.
				//Try it out later
				//int bcTaxRate = (*(int*)0x07837910);
				//psBC->sSettings.iTax = bcTaxRate;

				psBC->iClanID = 0;
				psBC->sSettings.iSkill = Dice::RandomI( 1, 3);
				psBC->sSettings.caGuardAmount[0] = 20;
				psBC->sSettings.caGuardAmount[1] = 20;
				psBC->sSettings.caGuardAmount[2] = 20;
				psBC->sSettings.saTowerType[0] = Dice::RandomI( 1, 3 );
				psBC->sSettings.saTowerType[1] = Dice::RandomI( 1, 3 );
				psBC->sSettings.saTowerType[2] = Dice::RandomI( 1, 3 );
				psBC->sSettings.saTowerType[3] = Dice::RandomI( 1, 3 );
				psBC->sSettings.saTowerType[4] = Dice::RandomI( 1, 3 );
				psBC->sSettings.saTowerType[5] = Dice::RandomI( 1, 3 );

				BLESSCASTLESERVER->NetSendBlessCastleStatus( NETSERVER->GetLoginServerConnection() ); //update login server
